...
public abstract record Result : IResult
{
    ...
    public static (T? Value, Error? Err) AsTuple<T>(Result result)
    {
        ...
        else
        {
            ...
            {
                false when result is Fail fail => (default, fail.Error),
                _ => throw new System.NotSupportedException("Result is in an impossible state."),
            };
        }
    }
    ...
    public static Result<T> AsResult<T>(Result result)
    {
        ...
        else
        {
            return result.IsSuccess switch
            {
                false when result is Fail fail => new Result<T>(fail.Error),
                _ => throw new System.NotSupportedException("Result is in an impossible state."),
            };
        }
    }
