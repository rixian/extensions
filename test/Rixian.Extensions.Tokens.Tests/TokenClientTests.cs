...
public class TokenClientTests
{
    ...
    [Fact(Skip = "Integration test requires valid token endpoint configuration.")]
    [Trait("TestCategory", "FailsInCloudTest")]
    public async Task TokenRetrievalWorks()
    {
        Result<ITokenClient> tokenClientResult = this.tokenClientFactory.GetTokenClient();
        ITokenClient tokenClient = tokenClientResult.Value!;
        Result<ITokenInfo> token = await tokenClient.GetTokenAsync();
    ...
    public async Task MultipleCallSameTokenTest()
    {
        Result<ITokenClient> tokenClientResult = this.tokenClientFactory.GetTokenClient();
        ITokenClient tokenClient = tokenClientResult.Value!;
        Result<ITokenInfo> token1 = await tokenClient.GetTokenAsync();
        Result<ITokenInfo> token2 = await tokenClient.GetTokenAsync();
    ...
    [Theory(Skip = "Integration test requires valid token endpoint configuration.")]
    [InlineData(10)]
    [InlineData(100)]
    [InlineData(1000)]
    [InlineData(10000)]
    ...
    public async Task ThreadSafetyTest(int count)
    {
        ...
        using (var resetEvent = new ManualResetEvent(false))
        {
            ...
            {
                resetEvent.WaitOne();
                return await tokenClientResult.Value!.GetTokenAsync();
            }
            ...
            Result<ITokenInfo>[] results = await Task.WhenAll(tasks);
